...
parser.add_option("-c", "--category", dest="category", default="all", type="string", help="Which set of nuisances to plot")
...
for i in range(fpf_s.getSize()):
    ...
    if not prefit.find(name):
      continue
    if options.category=='loose_stat':
	if 'loose_stat' not in name:
	  continue
    elif options.category=='tight_stat':
	if 'stat' not in name or 'loose_stat' in name:
	  continue
    elif options.category=='others':
	if 'stat' in name:
	  continue

    to_plot.append(name)

n_nuisances = len(to_plot)

pulls = []

nuis_p_i=0
# Also make histograms for pull distributions:
hist_fit_b  = ROOT.TH1F("prefit_fit_b"   ,"B-only fit Nuisances;;#theta ",n_nuisances,0,n_nuisances)
hist_fit_s  = ROOT.TH1F("prefit_fit_s"   ,"S+B fit Nuisances   ;;#theta ",n_nuisances,0,n_nuisances)
hist_prefit = ROOT.TH1F("prefit_nuisancs","Prefit Nuisances    ;;#theta ",n_nuisances,0,n_nuisances)
...
if options.plotfile:
    ...
    canvas_nuis.SaveAs(plotsDir+"pulls_%s.pdf"%options.category) 
    canvas_nuis.SaveAs(plotsDir+"pulls_%s.png"%options.category) 
